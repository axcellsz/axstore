...
function updateGlobalSummary() {
  const myEl = document.getElementById("summaryMyDebt");
  const custEl = document.getElementById("summaryCustDebt");
  if (!myEl && !custEl) return;

  let totalMyDebt = 0;
  let totalCustDebt = 0;

  // pakai SEMUA pelanggan, bukan yang sudah difilter
  customers.forEach((c) => {
    const { myDebt, customerDebt } = splitDebt(c.total);
    totalMyDebt += myDebt;
    totalCustDebt += customerDebt;
  });

  if (myEl) myEl.textContent = formatRupiah(totalMyDebt);
  if (custEl) custEl.textContent = formatRupiah(totalCustDebt);
}
...
function renderCustomers() {
  ...
  if (!filteredCustomers.length) {
    ...
    // kalau tidak ada pelanggan, ringkasan jadi 0
    updateGlobalSummary();
    return;
  }
  ...
  filteredCustomers.forEach((c, idx) => {
    ...
    // hitung hutang saya & hutang pelanggan untuk pelanggan ini
    const { myDebt, customerDebt } = splitDebt(c.total);
  ...
  // setelah list dirender, update ringkasan global
  updateGlobalSummary();
}
...
async function loadCustomers() {
  try {
    ...
    filteredCustomers = [...customers];
    applyFilter(); // ini juga akan memanggil renderCustomers -> updateGlobalSummary
  } catch (err) {
    console.error(err);
...
function renderDetail() {
  ...
  // pecah total pelanggan ini jadi 2: hutang saya & hutang pelanggan
  const { myDebt, customerDebt } = splitDebt(currentCustomer.total || 0);

  const detailMyEl = document.getElementById("detailMyDebt");
  const detailCustEl = document.getElementById("detailCustDebt");

  if (detailMyEl) detailMyEl.textContent = formatRupiah(myDebt);
  if (detailCustEl) detailCustEl.textContent = formatRupiah(customerDebt);
